...
def _host_platform_tag() -> str:
    ...
    try:
        ...
        raise RuntimeError(f"unsupported cleudocode-harness-runtime-bin build platform: {key}") from exc
...
class RuntimeBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict[str, object]) -> None:
        ...
        if self.target_name == "sdist":
            raise RuntimeError(
                "cleudocode-harness-runtime-bin is wheel-only; build and publish platform wheels only."
            )
        ...
        runtime_dir = Path(self.root) / "src" / "cleudocode_harness_runtime" / "runtime"
