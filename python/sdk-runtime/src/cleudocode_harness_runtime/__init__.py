...
PACKAGE_METADATA_FILENAME = "cleudocode-harness-runtime.json"
...
_EXE_ACQUISITION_HINT = (
    ...
    "deepseek-harness checkout, or install the matching `cleudocode-harness-runtime-bin` platform "
...
def bundled_package_dir() -> Path:
    ...
    if not metadata.is_file():
        raise FileNotFoundError(f"cleudocode-harness-runtime-bin is missing {metadata}")
    return root
...
def bundled_default_config_path() -> Path:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"cleudocode-harness-runtime-bin is missing the default runtime config at {path}"
        )
    return path
...
def bundled_runtime_path() -> Path:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"cleudocode-harness-runtime-bin is missing the runtime executable at {path}. "
            + _EXE_ACQUISITION_HINT
        )
    ripgrep = Path(f"{path}-rg")
    if not ripgrep.is_file():
        raise FileNotFoundError(
            f"cleudocode-harness-runtime-bin is missing the ripgrep sidecar at {ripgrep}. "
            + _EXE_ACQUISITION_HINT
        )
    if tag.startswith("macos-"):
        ...
        if not helper.is_file():
            raise FileNotFoundError(
                f"cleudocode-harness-runtime-bin is missing the node-pty spawn helper at {helper}. "
                + _EXE_ACQUISITION_HINT
            )
    return path
