...
from cleudocode_harness import CleudocodeHarness, HarnessClient, HarnessConfig, Notification, SdkProtocolError
...
""".strip()
    ...
    with CleudocodeHarness(launch_args_override=(sys.executable, str(script)), cwd=str(tmp_path)) as harness:
...
def test_public_signatures_omit_unsupported_wire_parameters() -> None:
    from cleudocode_harness import CleudocodeHarnessConfig, Session
    ...
    assert "profile" not in inspect.signature(CleudocodeHarness.run).parameters
    assert "profile" not in inspect.signature(Session.run).parameters
    assert "system_prompt" not in CleudocodeHarnessConfig.__dataclass_fields__
    assert "max_tokens" in CleudocodeHarnessConfig.__dataclass_fields__
...
""".strip()
    ...
    module_dir = tmp_path / "cleudocode_harness_runtime"
    module_dir.mkdir()
...
""".strip()
    ...
    monkeypatch.delitem(sys.modules, "cleudocode_harness_runtime", raising=False)
