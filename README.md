# Antigravity + Gemini CLI OAuth Plugin for cleudocode

[![npm version](https://img.shields.io/npm/v/cleudocode-antigravity-auth.svg)](https://www.npmjs.com/package/cleudocode-antigravity-auth)
[![npm beta](https://img.shields.io/npm/v/cleudocode-antigravity-auth/beta.svg?label=beta)](https://www.npmjs.com/package/cleudocode-antigravity-auth)
[![npm downloads](https://img.shields.io/npm/dw/cleudocode-antigravity-auth.svg)](https://www.npmjs.com/package/cleudocode-antigravity-auth)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![X (Twitter)](https://img.shields.io/badge/X-@dopesalmon-000000?style=flat&logo=x)](https://x.com/dopesalmon)

Enable cleudocode to authenticate against **Antigravity** (Google's IDE) via OAuth so you can use Antigravity rate limits and access models like `gemini-3-pro` and `claude-opus-4-5-thinking` with your Google credentials.
...
Paste this into any LLM agent (Claude Code, cleudocode, Cursor, etc.):

```
Install the cleudocode-antigravity-auth plugin and add the Antigravity model definitions to ~/.config/cleudocode/cleudocode.json by following: https://raw.githubusercontent.com/NoeFabris/cleudocode-antigravity-auth/dev/README.md
```

**Option B: Manual setup**

1. **Add the plugin** to `~/.config/cleudocode/cleudocode.json`:

   ```json
   {
     "plugin": ["cleudocode-antigravity-auth@latest"]
   }
   ```

   > Want bleeding-edge features? Use `cleudocode-antigravity-auth@beta` instead.

2. **Login** with your Google account:

   ```bash
   cleudocode auth login
   ```

3. **Add models** — choose one:
   - Run `cleudocode auth login` → select **"Configure models in cleudocode.json"** (auto-configures all models)
   - Or manually copy the [full configuration](#models) below

4. **Use it:**

   ```bash
   cleudocode run "Hello" --model=google/antigravity-claude-sonnet-4-5-thinking --variant=max
   ```
...
1. Edit the cleudocode configuration file at `~/.config/cleudocode/cleudocode.json`
...
Add this to your `~/.config/cleudocode/cleudocode.json`:

```json
{
  "$schema": "https://cleudocode.ai/config.json",
  "plugin": ["cleudocode-antigravity-auth@latest"],
  "provider": {
    "google": {
      "models": {
...
```bash
cleudocode auth login  # Run again to add more accounts
```

**Account management options (via `cleudocode auth login`):**
- **Configure models** — Auto-configure all plugin models in cleudocode.json
...
> **Quick Reset**: Most issues can be resolved by deleting `~/.config/cleudocode/antigravity-accounts.json` and running `cleudocode auth login` again.

### Configuration Path (All Platforms)

cleudocode uses `~/.config/cleudocode/` on **all platforms** including Windows.

| File | Path |
|------|------|
| Main config | `~/.config/cleudocode/cleudocode.json` |
| Accounts | `~/.config/cleudocode/antigravity-accounts.json` |
| Plugin config | `~/.config/cleudocode/antigravity.json` |
| Debug logs | `~/.config/cleudocode/antigravity-logs/` |

> **Windows users**: `~` resolves to your user home directory (e.g., `C:\Users\YourName`). Do NOT use `%APPDATA%`.

> **Custom path**: Set `cleudocode_CONFIG_DIR` environment variable to use a custom location.

> **Windows migration**: If upgrading from plugin v1.3.x or earlier, the plugin will automatically find your existing config in `%APPDATA%\cleudocode\` and use it. New installations use `~/.config/cleudocode/`.
...
1. Delete the accounts file:
   ```bash
   rm ~/.config/cleudocode/antigravity-accounts.json
   ```
2. Re-authenticate:
   ```bash
   cleudocode auth login
...
1. **Update to latest beta:**
   ```json
   { "plugin": ["cleudocode-antigravity-auth@beta"] }
   ```
...
3. Report the specific MCP in a [GitHub issue](https://github.com/NoeFabris/cleudocode-antigravity-auth/issues)
...
### Using with Oh-My-cleudocode

**Important:** Disable the built-in Google auth to prevent conflicts:

```json
// ~/.config/cleudocode/oh-my-cleudocode.json
{
  "google_auth": false,
  "agents": {
...
**Workaround:**
1. Stop cleudocode
2. Clean up: `rm ~/.config/cleudocode/*.tmp`
...
OAuth callback requires browser to reach `localhost` on the machine running cleudocode.
...
1. Ensure the plugin is installed: `"plugin": ["cleudocode-antigravity-auth@beta"]`
2. Copy `~/.config/cleudocode/antigravity-accounts.json`
...
{
  "plugin": [
    "cleudocode-antigravity-auth@latest",
    "@tarquinen/cleudocode-dcp@latest"
  ]
}
```
...
Disable built-in auth and override agent models in `oh-my-cleudocode.json`:
...
Create `~/.config/cleudocode/antigravity.json` for optional settings:

```json
{
  "$schema": "https://raw.githubusercontent.com/NoeFabris/cleudocode-antigravity-auth/main/assets/antigravity.schema.json"
}
```
...
```bash
cleudocode_CONFIG_DIR=/path/to/config cleudocode  # Custom config directory
cleudocode_ANTIGRAVITY_DEBUG=1 cleudocode         # Enable debug logging
cleudocode_ANTIGRAVITY_DEBUG=2 cleudocode         # Verbose logging
```
...
- [cleudocode-gemini-auth](https://github.com/jenslys/cleudocode-gemini-auth) by [@jenslys](https://github.com/jenslys)
