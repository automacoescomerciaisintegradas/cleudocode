...
SDK_DISTRIBUTION = "cleudocode-harness-sdk"
RUNTIME_DISTRIBUTION = "cleudocode-harness-runtime-bin"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="dsh-python-release-") as temporary:
        ...
        if args.package == "sdk":
            ...
            environment = None
            expected = output_dir / f"cleudocode_harness_sdk-{wheel_version}-py3-none-any.whl"
        else:
            ...
            expected = output_dir / f"cleudocode_harness_runtime_bin-{wheel_version}-py3-none-{platform_tag}.whl"
...
def stage_sdk(destination: Path, version: str) -> None:
    ...
    text, count = re.subn(
        r'"cleudocode-harness-runtime-bin==[^"]+"',
        f'"cleudocode-harness-runtime-bin=={version}"',
        pyproject.read_text(),
        count=1,
    )
    if count != 1:
...
def stage_runtime(destination: Path, version: str, executable: Path, executable_name: str) -> None:
    ...
    runtime_dir = destination / "src" / "cleudocode_harness_runtime" / "runtime"
