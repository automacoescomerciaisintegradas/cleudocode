...
## 1. Visão Geral
Este documento define o roteiro para implementar a arquitetura de "Gateway (Control Plane)" baseada em WebSocket (porta 18900) e o comando de instalação do daemon `onboard --install-daemon`.
...
2.  **WebSocket Control Gateway (`gateways/websocket_gateway.py`)**:
    *   Servidor WebSocket ouvindo em `127.0.0.1:18900`.
...
### Fluxo de Dados
[Cliente] <-> (WebSocket JSON-RPC) <-> [Gateway 18900] <-> [Daemon Router] <-> [Integrações (WhatsApp, etc)]
...
1.  **Criar `gateways/websocket_control.py`**:
    ...
    *   Utilizar `websockets` ou `aiohttp` (assíncrono) para servir na porta 18900.
...
1.  **Teste de Conexão**:
    *   Criar script de teste `scripts/test_ws_connection.py` que conecta no `ws://127.0.0.1:18900` e envia um "ping".
...
*   [ ] O daemon escuta na porta 18900.
