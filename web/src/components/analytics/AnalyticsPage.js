...
function AnalyticsPage() {
    ...
    const MetricCard = ({ metric }) => (<div className="cleudo-card animate-fade-in">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="p-2 bg-cleudo-accent-blue/20 rounded-lg">
          {metric.label.includes('Conversas') && <lucide_react_1.MessageSquare className="h-4 w-4 text-cleudo-accent-blue"/>}
          {metric.label.includes('Tokens') && <lucide_react_1.Zap className="h-4 w-4 text-cleudo-accent-blue"/>}
          {metric.label.includes('Tempo') && <lucide_react_1.Clock className="h-4 w-4 text-cleudo-accent-blue"/>}
          {metric.label.includes('Taxa') && <lucide_react_1.TrendingUp className="h-4 w-4 text-cleudo-accent-blue"/>}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className={`flex items-center gap-1 text-sm ${metric.positive ? 'text-cleudo-accent-green' : 'text-cleudo-primary'}`}>
          <lucide_react_1.TrendingUp className={`h-4 w-4 ${!metric.positive ? 'rotate-180' : ''}`}/>
          <span>{metric.positive ? '+' : '-'}{metric.change}%</span>
          <span className="text-text-muted ml-1">vs período anterior</span>
        </div>
      </div>
    </div>);
    const SimpleChart = ({ data, title, dataKey, color }) => (<div className="cleudo-card">
      ...
      <div className="h-64 flex items-end justify-between gap-1 p-4 bg-bg-dark/50 rounded-lg">
        {data.slice(-14).map((item, index) => {
            ...
            return (<div key={index} className="flex-1 flex flex-col items-center gap-2">
              <div className={`w-full rounded-t transition-all duration-500 delay-${index * 50}`} style={{
                    height: `${height}px`,
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <select value={timeRange} onChange={(e) => setTimeRange(e.target.value)} className="px-4 py-2 bg-bg-card border border-surface-border rounded-lg text-text-primary focus:outline-none focus:border-cleudo-accent-blue">
          ...
          <div className="flex gap-2">
            <button onClick={() => exportData('csv')} className="cleudo-btn cleudo-btn--outline">
              <lucide_react_1.Download className="h-4 w-4"/>
              CSV
            </button>
            <button onClick={() => exportData('pdf')} className="cleudo-btn cleudo-btn--primary">
              <lucide_react_1.Download className="h-4 w-4"/>
              PDF
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {metrics.map((metric, index) => (<div key={metric.label} className={`delay-${index * 100}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <SimpleChart data={chartData} title="Conversas por Dia" dataKey="conversations" color="#5B7CFF"/>
        <SimpleChart data={chartData} title="Tokens Processados" dataKey="tokens" color="#10b981"/>
      </div>

      <div className="grid grid-cols-1 gap-6">
        <SimpleChart data={chartData} title="Tempo de Resposta (ms)" dataKey="responseTime" color="#ef4444"/>
      </div>
      ...
      <div className="cleudo-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 bg-bg-dark/50 rounded-lg">
            <div className="flex items-center gap-3">
              <lucide_react_1.Users className="h-5 w-5 text-cleudo-accent-blue"/>
              <div>
          ...
          <div className="p-4 bg-bg-dark/50 rounded-lg">
            <div className="flex items-center gap-3">
              <lucide_react_1.MessageSquare className="h-5 w-5 text-cleudo-accent-green"/>
              <div>
          ...
          <div className="p-4 bg-bg-dark/50 rounded-lg">
            <div className="flex items-center gap-3">
              <lucide_react_1.Activity className="h-5 w-5 text-cleudo-accent-orange"/>
              <div>
